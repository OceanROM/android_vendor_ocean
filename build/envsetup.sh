function __print_ocean_functions_help() {
cat <<EOF
Additional oceanOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- oceangerrit:   A Git wrapper that fetches/pushes patch from/to oceanOS Gerrit Review.
- oceanrebase:   Rebase a Gerrit change and push it again.
- oceanremote:   Add git remote for oceanOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for oceanOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the ocean model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch ocean_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/ocean-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.ocean.device | grep -q "$ocean_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $ocean_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function oceanremote()
{
    ...
    git remote rm ocean 2> /dev/null
    ...
    local ocean_USER=$(git config --get review.review.oceanos.org.username)
    if [ -z "$ocean_USER" ]
    then
        git remote add ocean ssh://review.oceanos.org:29418/$PFX$PROJECT
    else
        git remote add ocean ssh://$ocean_USER@review.oceanos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'ocean' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/oceanOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.ocean.device | grep -q "$ocean_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        oceanremote
        git push ocean HEAD:refs/heads/'$1'
    fi
    '
}

function oceangerrit() {
    ...
    local user=`git config --get review.review.oceanos.org.username`
...
function oceanrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "oceanOS Gerrit Rebase Usage: "
        echo "      oceanrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.oceanos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.ocean.device | grep -q "$ocean_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/ocean/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $ocean_FIXUP_COMMON_OUT ]; then
