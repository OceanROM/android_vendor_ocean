...
#           (C) 2017-2018,2020-2021, The oceanOS Project
#
...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from oceanOS Github (http://github.com/oceanOS)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:oceanOS+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('oceanOS/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: oceanOS/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "oceanOS/%s" % repo_name })
...
print("Repository for %s not found in the oceanOS Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
