...
func init() {
	android.RegisterModuleType("ocean_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = oceanExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = oceanExpandVariables(ctx, dep_file)
	...
	cmd := oceanExpandVariables(ctx, String(g.properties.Cmd))
